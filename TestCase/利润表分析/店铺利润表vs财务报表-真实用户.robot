...
真实用户-店铺利润表vs财务报表
    ...
    ${ruleId}    获取规则    ${cookies}
    #店铺利润表接口
    ${response}    发送请求    ${cookies}    ${domain}/kmzk/profit/report/shop?api_name=kmzk_profit_report_shop&sysStatus=${sysStatus[0]}&startTime=${date_start}&endTime=${date_end}&shopUniIds=${shopUniId}&timeOrderType=asc&showDimension=0&ruleId=${ruleId}&showSuit=1&refundSumType=0&consignBeforeRate=&consignAfterRate=&isTrusted=true    None
    ...
    ${response_erp}    发送请求    ${cookies}    ${domain}/report/sale/dimensions/finance/list    ${param_erp}
